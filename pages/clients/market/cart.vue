...
export default {
  ...
  methods: {
    myValueChanged(value, itemId) {
      ...
      this.myOrder.tax = (this.myOrder.subTotal * this.tvaMarket) / 100;
      this.myOrder.total = this.myOrder.subTotal + this.myOrder.tax;
      persistData("market", this.myOrder);
    },
    removeItem(itemId) {
      let myNexArticles = [];
      for (let i = 0; i < this.myOrder.items.length; i++) {
  ...
  async mounted() {
    if (!getData("market")) {
      ...
      try {
        ...
        let tvaMarket = await axios.get(
          process.env.baseUrl + "/generalsettingsdefaults"
        );

        this.tvaMarket = tvaMarket.data.tvaMarket;
        this.myOrder.tax = (this.myOrder.subTotal * this.tvaMarket) / 100;
        this.myOrder.total = this.myOrder.subTotal + this.myOrder.tax+this.myOrder.deliveryPrice;
        persistData("market", this.myOrder);
      } catch (error) {
        console.log(error);
      }
