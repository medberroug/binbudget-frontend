  ...
  <div>
    ...
    <center v-if="myOrders.length == 0">
      <h5 class="text-muted m-5">
        Vous n'avez pas de commandes archivées pour le moment
      </h5>
    </center>
          ...
          <ul class="list-inline mb-0">
            <li class="list-inline-item">
              <a
                ...
                title="Consulter"
              >
                <nuxt-link :to="'/clients/orders/archive/' + data.item.id">
  ...
  async mounted() {
    ...
    try {
      ...
      for (let i = 0; i < result.length; i++) {
          //  console.log(  result.status[result.status.length - 1].name);
        if (
